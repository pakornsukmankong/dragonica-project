...
- Database schema/migrations live in `backend/supabase/migrations/` (`001`–`020`, applied in order via the Supabase SQL Editor). Tables: `profiles` (incl. `role`), `classes`, `characters`, `dungeons`, `items`, `sessions`, `session_drops`, `donations` (incl. `provider`, `hide_amount`), `tickets`, `ticket_messages`, `site_counters`, `skill_classes`, `skills`, `skill_builds`, `skill_build_likes`, `skill_build_comments`. (`maps` from 001 is dropped by 006.)
