...
/**
 * Sign-in / sign-up form, without any surrounding card or page chrome — the
 * caller supplies those. Shared by the /login page and the login modal the nav
 * opens when a guest picks a members-only destination.
 *
 * `next` is where a successful sign-in lands; callers must have validated it as
 * an internal path (anything else would be an open redirect).
 */
export function LoginForm({
  ...
  next: string;
  onSignedIn?: () => void;
  /** Lets a caller mirror the heading — the modal titles itself with it. */
